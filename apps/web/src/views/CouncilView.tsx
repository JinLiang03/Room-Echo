...
import { personaFor, withoutPersonaNames } from "../lib/personas";
  ...
  return (
    <li className="claim-row">
      <AgentDigitField
        role={claim.role}
        seed={claim.claim_id}
        label={`${claim.role.toUpperCase()} Agent 的数字角色视觉隐喻；不表示现场物体识别`}
      />
      ...
      <p className="claim-proposition">{withoutPersonaNames(claim.proposition)}</p>
...
function SystematicReadingBlock({
  ...
  return (
    <section className="systematic-reading" aria-label="系统解读">
      ...
      <p className="reading-scene">{withoutPersonaNames(reading.scene_sketch)}</p>
        ...
        {reading.layers.map((layer) => (
          <li key={layer.signal} className={`reading-layer reading-${layer.signal}`}>
            <div className="reading-layer-head">
              ...
              <span className="reading-metaphor">{withoutPersonaNames(layer.metaphor)}</span>
            </div>
            <p className="reading-explanation">{withoutPersonaNames(layer.explanation)}</p>
          </li>
        ))}
      </ol>
      {(reading.narrative || reading.boundary_notes?.length || reading.multimodal_hints?.length) ? (
        <details className="reading-more">
          ...
          <p className="reading-narrative">{withoutPersonaNames(reading.narrative ?? "")}</p>
          {reading.boundary_notes?.length ? (
            <ul className="reading-boundaries">
              {reading.boundary_notes.map((note) => (
                <li key={note}>{withoutPersonaNames(note)}</li>
              ))}
            </ul>
          ) : null}
          <div className="reading-hints">
            <ul>
              {reading.multimodal_hints?.map((hint) => (
                <li key={hint}>{withoutPersonaNames(hint)}</li>
              ))}
            </ul>
          </div>
        </details>
