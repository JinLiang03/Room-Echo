...
export const SPATIAL_THEMES: readonly SpatialTheme[] = [
  ...
  },
  {
    ...
    agentRole: "fusion",
    accent: "#c445b8",
    description: "非拟人的闭合数字流形；不表示人物、人体、姿态、身份或现场物体识别。",
  },
] as const;
...
function pointOnSegments(
  ...
  const scaled = Math.min(segments.length - 1, Math.floor(selector * segments.length));
...
/**
 * Three offset closed ribbons form a deliberately non-anthropomorphic field:
 * there is no head, torso, limb, face, or standing-axis grammar.
 */
function abstractPresencePoint(pick: number, u: number, v: number, w: number) {
  const ribbon = Math.min(2, Math.floor(pick * 3));
  const phase = [0.18, 2.24, 4.36][ribbon];
  const centerX = [-0.26, 0.24, 0.02][ribbon];
  const centerY = [0.18, 0.03, -0.24][ribbon];
  const radiusX = [0.58, 0.4, 0.68][ribbon];
  const radiusY = [0.24, 0.48, 0.31][ribbon];
  const theta = u * Math.PI * 2 + phase;
  const thickness = 1 + (v - 0.5) * 0.14;
  return {
    x:
      centerX +
      Math.cos(theta) * radiusX * thickness +
      Math.sin(theta * 3 + phase) * 0.055,
    y:
      centerY +
      Math.sin(theta) * radiusY * thickness +
      Math.cos(theta * 2 + phase) * 0.07,
    z: (w - 0.5) * 0.52 + Math.sin(theta * 2 + phase) * (0.15 + ribbon * 0.035),
  };
}
