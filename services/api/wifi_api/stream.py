...
from .config import PUBLIC_REPLAY_FORBIDDEN_DETAIL, public_replay
...
class StreamSession:
    ...
    def __init__(
        ...
        *,
        ...
        event_log_dir: Path | None = STREAM_LOG_DIR,
        ...
        self._latest_inference_evidence: EvidencePacket | None = None
    ...
    def _reset_timeline_view(self, revision: int) -> None:
        ...
        self._latest_inference_evidence = None
        self._last_result = None
        self._snapshot_state_events.clear()
        if self._runtime is not None:
            self._runtime.orchestrator.reset_continuity(self.session_id)
    ...
    @property
    def latest_inference_evidence(self) -> EvidencePacket | None:
        """Return the latest compact packet that passed the sensing gate."""
        if self._latest_inference_evidence is None:
            return None
        return self._latest_inference_evidence.model_copy(deep=True)

    @property
    def council_runtime(self) -> CouncilRuntime | None:
        """Return the runtime that owns this session's streamed council cycles."""
        return self._runtime or self._council_runtime
    ...
    async def _run(self) -> None:
        ...
        try:
            ...
            runtime.orchestrator.set_progress_sink(self._on_council_progress)
            ...
            async for frame in source.frames():
                ...
                for window in pipeline.transform([frame], manifest):
                    ...
                    periodic_analysis = (
                        ...
                        and now_s - last_seal_s
                        >= self.council_config.analysis_refresh_s
                    )
                    if (
                        trigger.should_seal(
                            triplet,
                        ...
                        if packet.signals.status == "ok":
                            self._latest_inference_evidence = packet.model_copy(deep=True)
                        self._seal_count += 1
                        ...
                        self._emit(
                            "cycle.started",
                            {
                                ...
                                "sequence": sequence,
                                "signal_snapshot": triplet.model_dump(mode="json"),
                                "analysis_refresh_s": self.council_config.analysis_refresh_s,
                            },
                        )
                        self._emit(
                            "quality.update",
                ...
                if trigger.should_seal(
                    ...
                    self._emit(
                        "cycle.started",
                        {
                            ...
                            "evidence_hash": packet.evidence_hash,
                            "sequence": sequence,
                            "signal_snapshot": stale.model_dump(mode="json"),
                            "analysis_refresh_s": self.council_config.analysis_refresh_s,
                        },
                    )
                    self._emit(
    ...
    def _on_council_progress(
        self,
        event_type: str,
        payload: dict[str, Any],
    ) -> None:
        """Forward safe, policy-screened role progress from the background cycle."""
                            ...
                            json.dumps(
                                {
                                    "type": "error",
                                    "status": 403,
                                    "detail": PUBLIC_REPLAY_FORBIDDEN_DETAIL,
                                },
                                ensure_ascii=False,
                            )
                        )
