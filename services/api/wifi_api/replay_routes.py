...
def _forbid_public_mutation() -> None:
    """Keep an internet-facing Replay deployment supervisor-owned and read-only."""
    if public_replay():
        raise HTTPException(
            status_code=403,
...
def _bundles() -> list[Path]:
    ...
    if public_replay():
        return [path for path in bundles if path.name == PUBLIC_REPLAY_BUNDLE_ID]
    return bundles
...
def bundle_detail(bundle_id: str) -> BundleSummary:
    if public_replay() and bundle_id != PUBLIC_REPLAY_BUNDLE_ID:
