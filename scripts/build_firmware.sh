...
for project in csi_tx csi_rx; do
  ...
  SIZE_OUT="$(cd "$ROOT/firmware/$project" && idf.py size 2>/dev/null || true)"
  SIZE_VAR="SIZE_$(echo "$project" | tr '[:lower:]' '[:upper:]')"
  export "$SIZE_VAR=$SIZE_OUT"
done
...
for project in ("csi_tx", "csi_rx"):
    manifest["sizes"][project] = os.environ.get(f"SIZE_{project.upper()}", "")
