...
test("home consumes the replay triplet as one signal-driven digital life", async ({
  ...
  await expect(
    page.getByLabel("数据源：SIM REPLAY 回放数据，非实时硬件"),
  ).toBeVisible({ timeout: 30_000 });
  const lifeField = page.locator(".digit-field");
  await expect(lifeField).toHaveAttribute("data-life-active", "true", {
    timeout: 30_000,
  });
  await expect(
    page.getByRole("button", { name: /点击或长按保存这一刻/ }),
  ).toBeVisible();
  await expect(page.locator(".agent-voice-item")).toHaveCount(7);
  await expect(page.locator(".agent-voice-item .has-voice").first()).toBeVisible({
    timeout: 60_000,
  });
  const snapshots = page.locator(".agent-voice-snapshot:not(.is-unavailable)");
  await expect(snapshots).toHaveCount(7);
  const seals = await snapshots.evaluateAll((elements) =>
    elements.map((element) => ({
      cycleId: element.getAttribute("data-cycle-id"),
      evidenceHash: element.getAttribute("data-evidence-hash"),
      windowId: element.getAttribute("data-window-id"),
    })),
  );
  expect(new Set(seals.map((seal) => seal.cycleId)).size).toBe(1);
  expect(new Set(seals.map((seal) => seal.evidenceHash)).size).toBe(1);
  expect(new Set(seals.map((seal) => seal.windowId)).size).toBe(1);
  expect(seals[0]?.evidenceHash).toMatch(/^sha256:/);
  ...
  const before = await lifeField.getAttribute("data-life-state");
  await page.mouse.move(600, 430);
  const after = await lifeField.getAttribute("data-life-state");
