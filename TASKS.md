...
### Competition submission preview — non-final (updated 2026-08-13)
...
- [x] Replace the seven public role cards with one visible Room Echo Agent;
  retain the existing rainbow-number inference field on the right, add the
  Agent's live explanation at upper left, and add a separate action receipt
  below it across desktop and mobile
- [x] Add the versioned deterministic `AgentActionDecision` contract, bind it
  to the sealed cycle/evidence hash and confidence cap, and keep Mock/Replay
  limited to simulated preview or withheld while Live remains withheld without
  a configured actuator adapter
- [x] Keep the seven-role Council available only as explicit audit detail and
  prevent provider free text from selecting or executing an action
- [x] Build the six-page semifinal deck and the 1:58, eight-shot demo-video
  storyboard with per-shot prompts, truthful current/next-MVP boundaries and
  a separate record of J's verified feedback versus pending second use
- [x] Pass the single-Agent local candidate gates: 351 Python non-hardware
  tests with 2 credential-gated skips, 51 contract tests, 100 Web tests,
  lint/typecheck/build, 40 offline browser cases with 2 redundant captures
  skipped, 16 desktop/mobile layout cases and 10 full-stack Replay Playwright
  cases
- [x] Deploy runtime commit `76c71b1` and externally verify the 2026-08-13
  single-Agent candidate at the public HTTPS/WSS preview URL: health and
  hash-bound simulated action passed; desktop/mobile show one Agent plus the
  retained rainbow-number field with zero console errors; official MCP
  completed a separate 10-call DeepSeek Council with `status=ready` and
  `cycle_status=ambiguous`
- [x] Match the supplied reference layout without adding visible data modules:
  one compact Agent card, four 2×2 reaction tiles, glass treatment, retained
  central digit morph and no perimeter digits; restore Memory/Why original
  structure while aligning material only
- [x] Make default `#/home` one accelerated synthetic care day: load all four
  ordered moments once, cycle every 8 seconds without navigation or re-fetch,
  and keep `?care=...` only as a deterministic initial-frame/test hook
- [x] Bind each care moment's v2 hash-scoped Mock `proxy_triplet` to the same
  public Agent, four reaction tiles and right inference field; fail closed
  together on malformed/degraded/unknown evidence without leaking Replay state
- [x] Harden care evidence refs against non-existent/unhashed paths, fail
  closed on degraded required observations, enforce ordered non-overlapping
  timelines, and pass 407 Python non-hardware tests, 114 Web tests, 102 contract
  tests, 44 Playwright cases with 2 redundant captures skipped,
  schema/type/fixture drift, static checks and desktop/mobile review
- [x] Deploy and externally reverify this exact reference-style/care candidate;
  Render `room-echo` is live at
  `https://wifi-spatial-council-replay.onrender.com/#/home` on deployment
  commit `ef74d66`; public DOM checks confirm same-frame Agent/action/field
  binding and the automatic 8-second switch
