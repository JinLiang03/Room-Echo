...
- Phase: 12 — hardening, audit, release & handoff (the historical Replay
  baseline is complete; competition submission preparation is in progress)
- Status: **submission_preview_candidate (non-final)**. This working version
  must not be treated as the locked contest ZIP or merged directly to `main`;
  live remains pending Phase 11 hardware.
- Source mode validated: mock + replay; live implemented with fake-transport
  tests, not hardware validated
- Hardware validated: no

## Single public Agent competition candidate (2026-08-13, non-final)

- The reference-style refinement is now deliberately minimal: Home contains
  only the Room Echo wordmark, three-segment navigation, one compact Agent
  card, four 2×2 reaction tiles, and the existing central rainbow-number field.
  The perimeter digit river is disabled; no resident, floor-plan, timeline,
  scenario picker, source table, or care-evidence module is visible.
- `Memory` and `Why` retain their pre-refinement information structure and
  footer; only their background, panel blur, border, shadow, typography and
  navigation treatment were aligned to the Home glass language.
- Default `#/home` is now one accelerated, continuous simulated care day. It
  loads the complete ordered fixture once, starts at routine, then cycles
  routine → bathroom timeout → manual fall drill → externally labelled
  night-pet activity every 8 seconds without navigation, URL mutation or
  re-fetching. `?care=...` remains only a deterministic initial-frame/test
  hook. The bottom system line reads `SIM · CARE` without adding another UI
  module.
- The care fixture remains synthetic and external-input-bound. Bathroom
  duration requires a simulated zone-presence label; pet activity requires a
  simulated external multisensor label; suspected-fall content is a manual
  drill label. Every tile visibly says simulated/withheld plus no connected
  light/speaker, no sent message, or no created robot task. Evidence refs must
  resolve into the canonical `care-evidence-core.v2`; each moment's embedded
  Mock `proxy_triplet` is part of that hash and binds the left Agent, four
  actions and right inference field to the same evidence/session/window.
  Malformed, unknown or degraded required inputs fail closed across all three
  regions instead of borrowing live/replay data. The scripted timeline remains
  ordered and non-overlapping.
- Latest local gates: Ruff; mypy across 80 source files; Python non-hardware
  **407 passed / 2 credential-gated provider smokes skipped**; contracts
  **102/102** with schema/type/fixture drift clean; Web lint 0 errors / 5
  existing Fast Refresh warnings, strict TypeScript, production build, and
  Vitest **114 tests**. Playwright passed **44 with 2 redundant captures
  skipped**; exact 1440×900 and 390×844 care screenshots were visually
  inspected, and final read-only review found no P0/P1 blocker.
- This exact candidate is deployed and externally reverified on the existing
  Render `room-echo` service at
  `https://wifi-spatial-council-replay.onrender.com/#/home`. Deployment
  commit `ef74d66` is live. The public Home shows `SIM · CARE`, one Agent,
  four action tiles and the inference field; DOM checks confirmed that all
  three regions share the same evidence hash/session/window and switch
  together after the 8-second interval.

- The Home evaluator surface now exposes one public `Room Echo Agent` instead
  of seven role cards. The seven-role Council remains the internal auditable
  reasoning layer and is shown only through the explicit `?audit=1` route.
- The current rainbow-number Canvas remains the product's live inference field.
  The competition redesign changes its composition rather than replacing it:
  desktop uses an approximately 31/69 split with the Agent's evolving
  explanation at upper left, its bounded action receipt below, and the existing
  signal-driven digit field on the right. Mobile stacks the same three regions.
- `AgentActionDecision` is a required, versioned, deterministic result bound to
  the sealed session, cycle and evidence hash. Its current allowlist is only
  `stay_silent`, `wait_and_observe`, or `ambient_light_preview`. Mock and Replay
  may report `SIMULATED PREVIEW` or `withheld`; Live remains `withheld` until an
  explicit safe actuator adapter exists. Provider prose cannot choose or
  execute an action.
- The public Agent and action window consume the exact sealed cycle snapshot;
  they do not borrow a newer signal triplet. Confidence remains capped by the
  raw sensor-confidence cap, and missing or mismatched evidence fails closed.
- Current sensing remains limited to activity, occupancy proxy, relative-depth
  proxy and quality. The candidate does not claim a person image, identity,
  pose, path, fall, night-time context, long-term routine learning or an
  acknowledged device action.
- Local candidate gates passed: Ruff; mypy across 77 source files; Python
  non-hardware **351 passed / 2 credential-gated provider smokes skipped**;
  contracts **51/51** with generated schema/type/fixture drift clean; Web lint
  0 errors / 5 existing Fast Refresh warnings, strict TypeScript, production
  build, and Vitest **26 files / 100 tests**; full-stack Replay Playwright
  **10/10**. Offline browser regression passed **40 with 2 redundant
  size-specific captures skipped**; the targeted desktop/mobile layout subset
  passed **16/16**, and 1440x900 plus 390x844 screenshots were visually
  inspected.
- Submission support artifacts now include a six-slide, approximately 3:30
  roadshow deck; an eight-shot, 1:58 video storyboard with a prompt for every
  shot; and a semifinal narrative covering need, target user, workflow,
  feedback, optimization, evidence and technical boundaries. J's first-use
  feedback is verified; the redesigned single-Agent second use remains pending.
  The elder-care direction is explicitly a next validation target rather than
  a completed or validated care product.
- Render deployed runtime commit `76c71b1` from `codex/submission-prep` at
  2026-08-13 02:54 CST. Public `/healthz` returned `status=ok` for API,
  collector, sensing, council and contracts. REST returned a hash-bound Replay
  `ambient_light_preview` with `execution_status=simulated_preview`,
  `decision_confidence=display_confidence=model_support=sensor_confidence_cap`,
  and explicit no-device wording.
- Public browser inspection at 1440x900 and 390x844 confirmed one visible
  Agent, its separate action receipt, the retained rainbow-number inference
  field, `SIM · REPLAY`, and `NOT A CAMERA IMAGE`; browser console errors were
  zero. The official MCP client negotiated protocol `2025-11-25`, listed only
  `get_system_health` and `invoke_room_echo`, and completed a separate bounded
  DeepSeek Council with 10 real calls, technical `status=ready`, and the honest
  semantic result `cycle_status=ambiguous`.
...
## Competition Agent continuity refinement (2026-08-10, non-final)

- The evaluator surface is now intentionally narrow: REST
  `POST /api/agent/invoke` plus two MCP tools, `get_system_health` and
  `invoke_room_echo`. The latter is correctly annotated as an external,
  non-destructive, idempotent provider call rather than read-only.
- The continuously looping public Replay stays on the deterministic Mock
  provider to avoid unbounded spend. A separate gated runner consumes only the
  latest integrity-verified compact EvidencePacket, executes one configured
  OpenAI or DeepSeek Council with a 10-call budget and bounded deadline, and
  caches the first successful result per process. It permits at most two paid
  attempts per process. Missing configuration returns 503; incomplete/deadline
  runs return a sanitized 502 and are never presented as successful.
- `scripts/verify_openai_full_council.py` validates real provider/model
  provenance, at least seven successful calls, propose/cross-examine/synthesize
  phase coverage, token/latency records, and the confidence cap before writing
  a sanitized report. It now supports `--provider deepseek` without changing
  the historical filename. The final public verification on runtime commit
  `281e761` passed at 2026-08-10 10:45 CST with `provider=deepseek`,
  `model=deepseek-v4-flash`, and **10 real `status=ok` calls** covering five
  propose calls, cross-examine, three responses, and synthesis. Render logs
  independently showed exactly ten matching Chat Completions HTTP 200s.
- An earlier run on `dd7b154` received eight provider HTTP 200s but failed the
  local complex proposal schema and correctly returned 502; it was not claimed
  as complete. The replacement adapter lets DeepSeek return only bounded role
  narrative, while the server deterministically rebinds measurements,
  reactions, role lens, evidence refs, analysis steps, challenge targets and
  multimodal parameters to the sealed packet. The final sanitized evidence is
  `artifacts/submission/deepseek-full-council-evidence.json`; its ambiguous
  result retained `display_confidence=0.369802 <= sensor_confidence_cap=0.739605`.
- Five proposer roles now fan out concurrently, followed by one skeptic and one
  fusion role. Approximately every seven seconds the stream seals the latest
  proxy snapshot without blocking signal/UI updates. Each role receives only
  its own previous Policy-approved interpretation plus a deterministic summary
  of current-vs-previous motion, occupancy, depth and quality changes. Memory is
  bounded to 32 sessions, rejected text is excluded, and Replay seek/reset
  clears continuity.
- Home selects claims from one coherent latest Council cycle and renders all
  seven roles against the exact sealed `signal_snapshot`. Activity, occupancy,
  relative depth and quality remain the only inputs to the numeric motion field;
  Fusion may select an allowlisted generative theme while retaining no
  measurement mutation path.
- Current local validation: Ruff passed; mypy passed for 70 source files;
  Python non-hardware **323 passed / 2 credential-gated provider smokes
  skipped**; Web lint 0 errors / 5 existing Fast Refresh warnings, strict
  TypeScript passed, Vitest **91/91**, production build passed; offline visual
  Playwright **40 passed / 2 size-specific cases skipped**; full-stack Replay
  Playwright **10/10**. Exact 1440×900 and 390×844 Home screenshots were
  inspected after the final wording change.
- A truthful first-use record now captures J's actual feedback that Agent
  presence was weak and text appeared unchanged, and ties it to the mount and
  latest-cycle fix. J explicitly confirmed the description, two quotes and
  personal need as truthful at 2026-08-09 23:11 CST, and authorized use under
  the alias J for registration, PPT, demo video and project display. The
  post-deploy second experience is still pending and is not represented as
  complete.

## Public status split and deployment (2026-08-10, non-final)

- Runtime commit `281e761` (including the status-split code from `e123e59`)
  separates technical delivery from semantic interpretation on both REST
  `POST /api/agent/invoke` and MCP `invoke_room_echo`: `status=ready` means the
  bounded request/provider Council completed; `cycle_status` preserves the
  Council conclusion (`supported`, `ambiguous`, or `unavailable`).
- Render is live at
  `https://wifi-spatial-council-replay.onrender.com`. Public `/healthz`
  returned `status=ok` with API, collector, sensing, council and contracts all
  `ok`. The official MCP client negotiated protocol `2025-11-25` and completed
  `invoke_room_echo` with `provider=deepseek`, `real_model_calls=10`,
  `quality=ok`, `status=ready`, and `cycle_status=ambiguous`.
- Render Free remains a platform-level cold-start risk: the service can sleep
  after inactivity and take about a minute to wake. `scripts/warm_render.py`
  plus `make warm-render URL=...` now provides a bounded pre-demo readiness
  check without invoking DeepSeek. Removing the sleep behavior itself
  requires a paid always-on instance; no paid plan was enabled automatically.

## Seven-role presentation contract refinement (2026-08-10, non-final)

- Four readable specialist projections are now explicit typed contracts:
  architecture maps only to `收紧/展开/阻断`, biota to `静息/惊跳/恢复`,
  feng shui to `聚/散/滞/冲`, and psyche to
  `安定/活跃/警觉/漂浮`. Their UI headings are narrative labels, while the
  analysis text is derived from the sealed motion, occupancy, relative-depth
  and quality snapshot and does not claim visual sensing.
- Soundscape no longer renders provider prose in the primary Web experience.
  `CouncilResult.sound_motion` deterministically translates the approved
  snapshot/agreement into rhythm, pitch, distance, thickness and synchrony.
  Skeptic exposes evidence sufficiency, whether judgment is withheld and the
  next validation. Fusion exposes a first-person spatial-life state plus a
  bounded interaction request through `life_interaction`.
- All seven same-cycle data observations are closed `<details>` controls by
  default. Legacy `该视角：` prefixes are stripped at the presentation edge.
  The Agent response overlay carries only colour, response effect and the
  five sound axes and declares `data-measurement-mutation=none`; Home still
  passes no Council result into the scalar signal-to-motion mapper, while
  Fusion may choose the target generative theme separately.
- The former thin lower line is now a large-digit sinusoidal river. Its speed
  and amplitude still come only from activity, density from occupancy, layers
  from relative depth and clarity/fracture from quality.
- Provider responsibilities are split into a small compatibility facade,
  shared protocol/call types, deterministic Mock and OpenAI implementation;
  Mock and DeepSeek now consume public helpers from `grounding.py` instead of
  importing private functions across modules. A documentation index separates
  current specifications from historical snapshots without deleting records.
- Local gates after the final split: Ruff passed; mypy passed for 75 source
  files; Python non-hardware **328 passed / 2 credential-gated provider smokes
  skipped**; contracts **44/44**; Vitest **91/91**; strict TypeScript and
  production build passed; lint has 0 errors / 5 pre-existing Fast Refresh
  warnings. In-app browser checks at 1440×900 and 390×844 confirmed root opens
  Home, all seven observations start closed, sound has five axes and no prose,
  the role headings are visible, the overlay cannot mutate measurement, and
  browser console errors are zero.
- GitHub branches `codex/submission-prep` and
  `codex/agent-continuity-deepseek` were fast-forwarded to code commit
  `b034b99`. Render deploy `dep-d9spji6gekts7394rk9g` reported that exact
  commit live at 2026-08-10 17:29 CST; public `/healthz` returned all five
  components `ok`, and desktop/mobile inspection confirmed the new headings,
  closed observations, no-prose sound axes, large-number river and zero
  browser errors.
- A fresh first-run DeepSeek Council completed at 2026-08-10 17:32 CST with
  `provider=deepseek`, `model=deepseek-v4-flash`, **10 real successful calls**
  across propose/cross-examine/respond/synthesize, `status=ready`, and the
  honest semantic result `cycle_status=ambiguous`. The confidence invariant
  remained `display_confidence=0.4675 <= sensor_confidence_cap=0.935`.
  The official MCP client negotiated protocol `2025-11-25`, listed only the
  two bounded tools, and verified all five specialist presentation contracts,
  `soundscape.presentation.analysis=null`, five skeptic assessments,
  five-axis `sound_motion`, and first-person `life_interaction`. Sanitized
  evidence is in
  `artifacts/submission/deepseek-full-council-evidence.json`.

## Agent-selected generative theme refinement (2026-08-10, non-final)

- Home now lets the approved Fusion `life_interaction.effect` select an
  allowlisted presentation theme: contract/block map to a floor-plan metaphor,
  expand to a seat/lounging metaphor, startle/surge/activate to a lamp,
  gathering to an atrium, and echo to a studio. Unavailable or missing results
  keep the signal-selected fallback.
- A fresh active Home session introduces `floorplan → volume → selected theme`
  and later Council theme changes morph through the digit river. The themes are
  generated visual metaphors, not furniture, rooms, or layouts detected by CSI.
- Home keeps `result=null` inside `mapRenderParams`; motion, occupancy, depth and
  quality still control numeric speed, density, layering and clarity. Fusion
  chooses only the target point template and cannot mutate SignalTriplet,
  EvidencePacket, confidence or the scalar mapping.
- Local validation after this change: Web **24 test files / 96 tests passed**,
  strict TypeScript passed, production build passed, lint 0 errors / 5 existing
  Fast Refresh warnings, and the in-app preview visibly reached `floorplan`,
  `volume`, then Fusion-selected `atrium` and `floorplan` stages with zero page
  console errors. The verified source update is published to both
  `codex/submission-prep` and `codex/agent-continuity-deepseek`; Render's
  service display name is now `room-echo` for the Room Echo project.

## Final pre-submission dynamic/static measurement (2026-08-10, non-final)

- Local static gates passed: Ruff, mypy (75 source files), Python non-hardware
  **328 passed / 2 credential-gated provider smokes skipped**, Web lint (0
  errors / 5 existing Fast Refresh warnings), strict TypeScript, Vitest
  **24 files / 96 tests**, and production build.
- Public Render health returned `status=ok` for API, collector, sensing,
  council and contracts. The official MCP client negotiated protocol
  `2025-11-25`, listed exactly `get_system_health` and `invoke_room_echo`, and
  completed both calls.
- Public real-provider proof completed at
  `artifacts/submission/deepseek-full-council-evidence-20260810.json`:
  `provider=deepseek`, `model=deepseek-v4-flash`, **10 real successful calls**,
  technical `status=ready`, semantic `cycle_status=ambiguous`, successful
  propose/cross-examine/respond/synthesize coverage, and
  `display_confidence=0.4675 <= sensor_confidence_cap=0.935`.
- The continuous Replay remains truthfully `provider=mock` with zero real
  calls; only the bounded evaluator-facing invoke path calls DeepSeek. This
  is the intended dynamic-measurement split, not a claim that the looping
  visual stream is live hardware sensing.
...
## Competition submission preview (2026-08-09; non-final)

- Added a same-origin production path: FastAPI can serve the built Web app,
  REST, WebSocket, Agent query endpoints, and Streamable HTTP MCP from one
  process. `PUBLIC_REPLAY=1` forces the sealed `demo_2min` Replay, auto-loop,
  read-only controls, one worker, no raw recording, and no per-loop event-log
  growth. Local ASGI production checks returned 200 for Web/health/Agent,
  403 for mutations, and completed an official MCP client handshake.
- Added a deliberately narrow evaluator MCP surface at `/mcp/` with two tools:
  `get_system_health` and the external, non-destructive, idempotent
  `invoke_room_echo`. It exposes provider provenance and truth boundaries and
  cannot start Live, mutate transport, inject faults, record raw CSI, or read
  server paths/secrets. Standard REST surfaces remain at `/api/agent/latest`,
  `/api/agent/query`, and the gated `/api/agent/invoke`.
- Unified the stream and REST Council runtime; repaired backward seek/paused
  step semantics, late-join hydration, source health recovery, latency
  calculation, finished-result retention, and single-sample Sparkline/Canvas
  behavior. Public Memory/Replay UI now respects the read-only capability.
- Hardened source packaging: deterministic tracked-file ZIP, embedded Git and
  per-file SHA-256 manifest, required Web/deployment/Agent/MCP paths, private
  submission worksheets and artifacts excluded, and pre/post extraction scans
  for machine paths. Firmware manifest, hardware inventory, and historical
  state no longer contain a concrete local user or serial-device path.
- Validation at that 2026-08-09 preview snapshot: Ruff passed; mypy passed for
  67 source files; contracts
  43/43; Python non-hardware **305 passed / 1 credential-gated OpenAI test
  skipped**; Web lint 0 errors / 5 Fast Refresh warnings, TypeScript passed,
  Vitest **79/79**, production build passed; Replay E2E **2 service + 10
  desktop/mobile Playwright passed**. Multimodal perf measured 60 FPS / 174
  draws / 1 dropped frame; the 900-point digit field measured 60 FPS / 1041
  draws / 0 dropped frames.
- Public Preview is live from commit `845f117` at
  `https://wifi-spatial-council-replay.onrender.com`. `/healthz` returned 200;
  the official MCP client negotiated protocol `2025-11-25`, listed both tools,
  and called both successfully. `get_system_health` truthfully reported the
  continuous loop as `provider=mock, real_model_calls=0`; the separately gated
  `invoke_room_echo` returned the cached verified run as
  `provider=deepseek, real_model_calls=10`. The free instance may cold-start
  after inactivity.
- This is intentionally **not the final submission**. Still pending: a real
  same-origin HTTPS/WSS preview and one credentialed full Council are now
  verified; J's public second-use confirmation, user-approved final
  copy/visuals, the exact clean contest commit, clean-room ZIP verification,
  and final contest lock remain pending. Docker configuration is built and live
  on Render; it has not been built locally because no container engine is
  present.
- Public preview defaults to deterministic Mock Council output so an infinite
  Replay loop cannot spend model tokens. Real DeepSeek execution is isolated to
  the gated invoke path and its first successful result is cached per process;
  Mock output is never claimed as real LLM validation.
- Live ESP32 capture, same-room calibration, held-out metrics, and 30-minute
  hardware stability remain `blocked_by_hardware`; no Live claim is made.
...
- Serial devices: only ordinary macOS system/debug ports plus one
  **unconfirmed** non-system serial device. System inspection showed no
  ESP32-class USB identity for it; it was NOT probed or flashed (unknown
  device, one port instead of three).
...
- Phase 11 is blocked: the three-board prerequisite is not met on this
  ...
  claim is made.
- Git is active. Competition work must be reviewed on a `codex/` preview
  branch; the final commit, merge to `main`, release ZIP, and contest lock are
  pending explicit user approval.
- `make dev` uses `make -j2`; Ctrl-C stops both processes (verified).

## Change log

- 2026-08-09: Prepared a non-final competition preview candidate: same-origin
  read-only Public Replay, standard Agent REST, five-tool Streamable HTTP MCP,
  unified Council state, production container/Render descriptors, deterministic
  privacy-scanned source ZIP tooling, product-facing docs, and current full
  Replay/Web/backend gates. At this preparation point, real full-Council
  provider proof, real-user validation, final clean ZIP, contest lock, and Live
  hardware claim were not recorded.
- 2026-08-09: Deployed the non-final Preview from `78f33af` to Render. Root,
  health, read-only mutation protection, and all five public MCP tool calls
  passed; the service remains a deterministic Mock Replay and is not the final
  contest version.
