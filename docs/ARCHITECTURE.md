...
flowchart LR
    ...
    EVI --> CR["Internal CouncilRuntime\n5 proposer + skeptic + fusion + policy"]
    CR --> ACT["Deterministic AgentActionDecision\npreview or withheld"]
    SIG --> HUB["StreamSession + StreamHub"]
    CR --> HUB
    ACT --> HUB
    CAREFIX["Deterministic care fixture\nsynthetic context + action intents"] --> CAREAPI["GET /api/care/scenario\nread-only demo input"]
    HUB --> REST["REST: stream / council / agent"]
    HUB --> WS["WebSocket /ws"]
    HUB --> MCP["Streamable HTTP /mcp/\n2 bounded tools"]
    REST --> WEB["React\n1 public Agent + action window + Canvas 2D"]
    WS --> WEB
    CAREAPI -. "default Home: one load / 8 s cycle" .-> WEB
```

实时信号链不等待 Agent。Council 只读取 `EvidencePacket` 中的标量摘要和三代理信号，不读取 raw CSI；Provider 离线或超时时，信号与 WebSocket 仍继续运行。前台只投影一个 Room Echo Agent；五个 proposer、skeptic 与 fusion 是内部审议角色，不是七个面向用户的 Agent。行动决策由服务器确定性生成，不读取 Provider 自由文本。养老模拟 fixture 走独立只读路径，不进入 sensing、Evidence 或 Council，也不把外部标签伪装成 Wi-Fi 结论。
...
| `services/council` | 五角色受限提案、skeptic 质询、回应、Policy、Fusion、store、受限展示字段、确定性行动决策与可选七角色审计数据 | 创造测量、写回 Evidence、用共识抬分、让 Provider 自由文本选择行动 |
| `services/api` | 单个进程内 `StreamSession`、恢复缓冲、REST、WebSocket、Agent/MCP 适配、只读养老模拟 endpoint 和可选静态 Web | 浏览器状态作为服务端真相、多进程共享会话、执行照护设备 |
| `packages/contracts` | Pydantic 单一结构源，生成 JSON Schema 与 TypeScript 类型，包含 `AgentActionDecision` 与 `SimulatedCareScenario` | 业务计算、把 synthetic 标签升级成测量 |
| `apps/web` | 连接/序列恢复、React reducer、左侧单一 Agent 与 2×2 四反应窗口、右侧 Canvas 2D 数字推断场、显式 audit、Web Audio、局部视觉书签 | 在浏览器重算正式信号、持有 API key、把图形称为相机图像、执行真实设备动作、把 care fixture 扩展成公开资料页 |
...
7. Council 完成 hash/质量门，并发执行五角色首轮提案，再执行 skeptic、必要回应、Policy 和可选 synthesis；安全的增量结果会在周期内逐步推送，最终 `CouncilStore.commit` 使用 sequence guard。Provider 协议、Mock、OpenAI 与 DeepSeek 分模块实现；`grounding.py` 为 Mock 与 DeepSeek 提供同一组公开确定性证据辅助函数，`presentation.py` 只负责受限展示投影，不写回测量。
8. `build_agent_action_decision` 只读取当前封存 packet、确定性 Council 状态与受限置信：Mock/Replay 的受支持周期可生成 `ambient_light_preview + simulated_preview`；有歧义/降级时生成 `wait_and_observe + withheld`；证据不足或契约失败时生成 `stay_silent + withheld`；Live 因没有已验证执行器适配器始终 withheld。这个决策作为 `CouncilResult.action_decision` 一同提交。
9. `StreamHub` 发送 snapshot 与实时事件；浏览器只按单调 sequence 应用，断线后用 `last_sequence` 恢复。首页把同一周期的 `signal_snapshot`、最终解释和 `action_decision` 投影为一个公开 Agent，避免跨周期拼接。

不使用跨 RX 相位、AoA、TDoA 或绝对 ToF；相对纵深不是米制距离。

### 4.1 独立养老模拟数据路径

养老演示由 `simulated-care-scenario.v2` 提供一个完全确定性的 24 小时 fixture：一名不含真实个人数据的匿名虚构老人、58㎡ 一室一厅六区户型、13 条活动脚本，以及日常、卫生间超时、人工跌倒演练、夜间宠物四个时刻。`GET /api/care/scenario?moment=routine|bathroom_timeout|fall_drill|pet_night` 一次返回完整一天，并用 `selected_moment` / `current_index` 标识 API 选择结果。默认 `#/home` 只加载一次完整响应，从日常开始按固定顺序每 8 秒自动循环，不切页、不改 URL、也不重新请求；`?care=...` 仅用于指定可复现的初始帧。公开首页不提供场景选择器，也不展示人物、户型、全天时间线或 provenance 信息模块。

这一条路径与正式 Wi-Fi Evidence 严格分离：

- 人物、户型、区域、时间线与照护阈值来自 synthetic fixture；
- 卫生间时长依赖模拟外部区域存在标签与模拟时钟；
- 宠物结论依赖 `simulated_external_multisensor_label`，Wi-Fi 单独不能区分人或宠物；
- 跌倒只来自 `simulated_manual_fall_drill_label`，是人工演练，不是已验证检测；
- 每个 `care-evidence-core.v2` 都内嵌唯一 Mock `proxy_triplet`；它属于 canonical hash，只代表活动、遮挡/空间占用、相对纵深三代理旁证，不能补出人物、房间、宠物、姿态或伤情；
- 四项建议分别是环境光预演、语音脚本、家属消息草稿、机器人任务预演，只能 `simulated_preview` 或 `withheld`，`device_execution_enabled=false`，没有设备 adapter、发送、ACK 或完成状态。

该合约仍约束 `action_confidence <= conclusion_confidence <= sensor_confidence_cap`。这里的 confidence chain 是模拟场景内的可审计上限，不是 Live 照护准确率或临床/安全验证结果。
...
| GET | `/api/care/scenario?moment=...` | 读取完整养老模拟日与四个确定性时刻；只读、无设备副作用 |
...
`synthesis.result` 内的 `CouncilResult.action_decision` 使用 `agent-action-decision.v1`。它绑定同一 `session_id`、`cycle_id` 与 `evidence_hash`，且满足 `decision_confidence <= sensor_confidence_cap`。合约故意不存在“真实已执行”状态：当前只允许 `simulated_preview` 与 `withheld`，没有浏览器或后端执行器会据此控制真实设备。
...
- 首页约以 31/69 分为左侧解释区与右侧实时数字推断场；左侧严格保留品牌、三个公开导航、一个简洁 Room Echo Agent 卡和下方 2×2 四反应窗口，不增加其他信息区；
- 首页、记忆与为什么使用同一毛玻璃面板、柔和背景与字形系统；Memory 仍是回放控制、事件时间线与视觉书签，Why 仍是当前判断、主要质疑和置信边界，只统一视觉而不改信息架构；四反应 tile 中的生成式“生物形态”只作 Agent 反应图标，不是人物、宠物或物种识别；
- 数字场只消费当前显示源的 signal/quality；普通模式使用 Stream triplet，默认 care 模式使用当前时刻 hash-bound Mock `proxy_triplet`。首页关闭四周循环数字外框，但保留中心彩色数字形态和实时动画。中心形态不是现场轮廓；care 数据不能写回正式 stream reducer、Council Evidence、测量或置信；
- 默认 care 首页把同一时刻的通俗结论、四个受限行动意图和 `proxy_triplet` 原子映射到现有 Agent 卡、四反应 tile 与右侧数字场；`?care=...` 只选择初始帧。它不得创建公开 selector、`SPACE / CONTEXT / INPUT`、人物/户型/时间线卡或 Why 来源分栏；
- `publicAgentPresentation` 只使用最新周期的封存 snapshot 和结果；stale/offline 时清除旧解释并显示 unknown，不把上一周期冒充“此刻”；
- 七角色 claim/challenge/response 只在显式 audit 视图展开，普通首页和“为什么”公共层不并列展示七个 Agent；
- `AgentActionWindow` 在普通 Wi-Fi 模式把一个 `CouncilResult.action_decision` 补齐为四个简洁反应位；默认 care 演示在相同四个位置渲染当前时刻的受限行动意图。两种模式都不连接执行器，也不增加信息模块；
...
所有生成图形必须标注为 inference field / not a camera image。普通流 unknown/stale，或 care 响应 malformed、unknown、必需外部观察/代理降级时，Agent、四行动与数字场同时清除当前驱动并显示 waiting/unavailable，不得借用旧 Replay 状态。UI theme、pointer、scroll、视觉书签、养老 fixture 和行动预览都不能反向修改正式测量。当前 Wi-Fi 系统不推断人物、跌倒、宠物、房间、路径、夜间状态或长期习惯；后端 care fixture 中的相关字段始终保留 synthetic / external / drill provenance，不能被描述为 Wi-Fi 结论。
