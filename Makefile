...
.PHONY: setup dev live api web test lint typecheck build verify-contracts fixtures schemas types demo submission-demo mcp-smoke warm-render verify-openai-council verify-deepseek-council firmware-build firmware-host-tests test-collector generate-fixtures verify-replay replay-smoke test-sensing-core benchmark-sensing extract-features test-calibration calibration-wizard evaluate-calibration test-signals replay-signals test-council replay-council test-confidence-invariants multimodal-perf-smoke e2e-replay fault-injection soak-replay hardware-sanity calibrate-live test-hardware compare-live-replay release-check
...
submission-demo:
	...
	npm --prefix $(WEB) ci
	npm --prefix $(WEB) run build
	SERVE_WEB=1 PUBLIC_REPLAY=1 APP_MODE=replay SCENARIO=demo_2min \
		DEMO_AUTOSTART=1 DEMO_LOOP=1 AGENT_PROVIDER=mock \
		$(UV) run uvicorn wifi_api.app:app --host 127.0.0.1 --port $(PORT) --workers 1

mcp-smoke:
	$(UV) run python scripts/mcp_smoke.py "$(MCP_URL)" --provider "$(MCP_PROVIDER)"

warm-render:
	@test -n "$(URL)" || (echo "URL=https://your-service.onrender.com is required" >&2; exit 2)
	$(UV) run python scripts/warm_render.py "$(URL)"

verify-openai-council:
	@test -n "$(URL)" || (echo "URL=https://your-service.example is required" >&2; exit 2)
	$(UV) run python scripts/verify_openai_full_council.py "$(URL)"

verify-deepseek-council:
	@test -n "$(URL)" || (echo "URL=https://your-service.example is required" >&2; exit 2)
	$(UV) run python scripts/verify_openai_full_council.py "$(URL)" --provider deepseek
