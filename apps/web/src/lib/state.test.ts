...
describe("streamReducer", () => {
  ...
  it("marks pause stale but treats a finished replay as a sealed final frame", () => {
    let state = initialState();
    state = apply(
      state,
      event(1, "session.status", {
  ...
  it("preserves the final triplet and council result after replay finishes", () => {
    let state = apply(
      initialState(),
      event(1, "session.status", {
        session_id: "session-test",
    ...
    state = apply(
      ...
      event(3, "synthesis.result", {
        ...
        result: {
          schema_version: "council-result.v1",
          cycle_id: "cycle-final",
          evidence_hash: "sha256:final",
          status: "supported",
          headline: "最终受限解读",
          summary: "sealed replay result",
          sensor_confidence_cap: 0.7,
          model_support: 0.6,
          display_confidence: 0.6,
          interpretation_agreement: {
    ...
    expect(state.stale).toBe(false);
    expect(state.triplet?.window_id).toBe(triplet.window_id);
    expect(state.council.cycles["cycle-final"]?.result?.headline).toBe(
      "最终受限解读",
    );
  });
  ...
  it("merges one truthful claim at a time for progressive Council streams", () => {
    let state = apply(
      initialState(),
      event(1, "cycle.started", {
        cycle_id: "cycle-progressive",
        evidence_hash: "sha256:progressive",
      }),
    );
    const baseClaim = {
      ...
      state: "proposed",
      stance: "supports",
      evidence_refs: ["evidence://sha256:progressive/signals/motion/value"],
    ...
    state = apply(
      ...
      event(3, "agent.claim", {
        ...
        claim: {
          ...baseClaim,
          claim_id: "claim-biota",
          agent_id: "agent-biota",
          role: "biota",
          proposition: "随后返回生长视角。",
        },
      }),
    );

    expect(
      state.council.cycles["cycle-progressive"]?.claims.map(
        (item) => item.proposition,
      ),
    ).toEqual(["先返回构造视角。", "随后返回生长视角。"]);
  });

  it("binds cycle.started to the SignalTriplet sealed for that EvidencePacket", () => {
    let state = apply(initialState(), event(1, "signal.frame", { triplet }));
    state = apply(
      state,
      event(2, "cycle.started", {
        cycle_id: "cycle-sealed",
        evidence_hash: "sha256:sealed",
        signal_snapshot: { ...triplet, window_id: "window-explicit-seal" },
        analysis_refresh_s: 7,
      }),
    );
    state = apply(
      state,
      event(3, "signal.frame", {
        triplet: { ...triplet, window_id: "window-after-seal" },
      }),
    );

    const sealed = state.council.cycles["cycle-sealed"];
    expect(sealed?.evidenceHash).toBe("sha256:sealed");
    expect(sealed?.signalSnapshot?.window_id).toBe("window-explicit-seal");
    expect(sealed?.analysisRefreshS).toBe(7);
    expect(state.triplet?.window_id).toBe("window-after-seal");
  });

  it("merges progressive challenge, response, and policy events", () => {
    let state = apply(
      initialState(),
      event(1, "cycle.started", {
    ...
    state = apply(
      ...
      event(3, "policy.rejection", {
        ...
        rejection: {
          ...
          agent_id: "agent-test",
          role: "architecture",
          reason_code: "forbidden_metric_depth",
          detail: "redacted test detail",
          rejected_at: "2026-08-10T00:00:00Z",
        },
      }),
    );

    expect(state.council.cycles["cycle-events"]?.challenges).toHaveLength(1);
    expect(state.council.cycles["cycle-events"]?.rejections).toHaveLength(1);
  });
