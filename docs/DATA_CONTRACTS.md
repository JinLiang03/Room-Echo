...
`simulated-care-scenario.v2` 是独立于正式 sensing、`EvidencePacket`、Prompt
和 Council 的只读 Mock 工作流合约。一次响应包含同一虚构日的完整时间线
和四个固定 `SimulatedCareMoment`；`simulation_only=true`、
`source_mode=mock`、`device_execution_enabled=false` 是不可放宽的不变量。

每个时刻包含一个 `care-evidence-core.v2`。该 core 内嵌唯一
`SignalTriplet proxy_triplet`，并整体进入 canonical SHA-256：

```text
evidence_hash = sha256(canonical_json(care_evidence_core_v2))
```

`proxy_triplet` 必须绑定同一 scenario/session、event、timeline entry、
window 与时间边界，且 `source_mode=mock`。事实引用与四项建议只能引用该
hash 下真实存在的 core 路径；四项建议固定为环境光预演、语音询问脚本、
家属消息草稿和机器人查看任务预演，并满足：

```text
action_confidence <= conclusion_confidence <= sensor_confidence_cap
execution_status in {simulated_preview, withheld}
external_execution_allowed = false
```

若 `proxy_triplet` 或必需外部观察降级，则
`interpretation_status=unknown`，结论置信不高于 `min(0.25,
sensor_confidence_cap)`，四项行动全部 `withheld`。Web 对 malformed、
unknown 或 degraded care payload 必须同时清除 Agent、行动与推断场驱动，
不得借用 live/replay 数据补全。
