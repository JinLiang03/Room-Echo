...
describe("WhyView", () => {
  it("keeps the latest interpretable result visible when a stale cycle ends the stream", () => {
    ...
    expect(container.querySelectorAll('[data-public-agent="room-echo"]')).toHaveLength(0);
    expect(container.querySelector(".council-view")).toBeNull();
  });
  ...
  it("does not expose care selectors or three-column care evidence", () => {
    const { container } = renderWithStream(
      <WhyView />,
      stateWithCycles([cycle("cycle-0001", "supported", "可解释结果")]),
    );

    expect(container.querySelector(".room-echo-page-lead")).toBeNull();
    expect(container.querySelectorAll('[data-public-agent="room-echo"]')).toHaveLength(0);
    expect(container.querySelector(".care-scenario-selector")).toBeNull();
    expect(container.querySelector(".why-care-evidence")).toBeNull();
  });
});
