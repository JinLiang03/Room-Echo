...
function applySessionStatus(
  ...
  return {
    ...next,
    session,
    // A completed Replay is a sealed final frame, not a disconnected sensor.
    // Keep its last triplet/result visible; pause and premature stop remain stale.
    stale: session.paused || (!session.running && !session.finished),
...
function applyEvent(state: StreamState, event: StreamEvent): StreamState {
  ...
  switch (event.event_type) {
    ...
    case "cycle.started": {
      ...
      const sealedSnapshot = event.payload.signal_snapshot as
        | StreamState["triplet"]
        | undefined;
      const analysisRefreshS = Number(event.payload.analysis_refresh_s ?? 0);
      const cycle: CycleView = existing
        ? {
            ...
            analysisRefreshS:
              analysisRefreshS > 0
                ? analysisRefreshS
                : existing.analysisRefreshS,
            signalSnapshot:
              existing.signalSnapshot ?? sealedSnapshot ?? next.triplet ?? undefined,
          }
        : {
            cycleId,
            evidenceHash: String(event.payload.evidence_hash ?? ""),
            startedAt: event.emitted_at,
            analysisRefreshS: analysisRefreshS > 0 ? analysisRefreshS : undefined,
            signalSnapshot: sealedSnapshot ?? next.triplet ?? undefined,
            claims: [],
            challenges: [],
            rejections: [],
            result: null,
    ...
    case "policy.rejection": {
      ...
      // The current backend sends a cycle snapshot (`claims`), while a live
      // provider may emit one `claim` at a time. Merge both forms by stable id
      // so Home can visibly update as each truthful Council record arrives.
      const claims = mergeCouncilRecords(
        cycle.claims,
        payloadRecords<CycleView["claims"][number]>(event.payload, "claims", "claim"),
        (claim) => claim.claim_id,
      );
      const challenges = mergeCouncilRecords(
        cycle.challenges,
        payloadRecords<CycleView["challenges"][number]>(
          event.payload,
          "challenges",
          "challenge",
        ),
      ...
      const rejections = mergeCouncilRecords(
        cycle.rejections,
        payloadRecords<CycleView["rejections"][number]>(
          event.payload,
          "rejections",
          "rejection",
        ),
        (rejection) => rejection.rejection_id,
      );
      return {
        ...next,
        council: {
...
function payloadRecords<T>(
  ...
  return singular && typeof singular === "object" ? [singular as T] : [];
}

function mergeCouncilRecords<T>(
  existing: T[],
  incoming: T[],
  ...
  const merged = new Map(existing.map((record) => [idFor(record), record]));
  for (const record of incoming) {
    merged.set(idFor(record), record);
  }
