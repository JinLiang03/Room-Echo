...
interface Props {
  ...
  debug?: boolean;
  autoMorph?: boolean;
  /** Presentation-only rectangular digit perimeter around the centre form. */
  showPerimeter?: boolean;
  /** Home-only fluid particle treatment. It never changes signal mapping. */
  fluid?: boolean;
  /**
   * A presentation vocabulary selected from the already-bound Agent frame.
   * It changes only the motion grammar of the inference field, never its
   * measurements, confidence, evidence binding, or sensing claims.
   */
  fluidMode?: FluidVisualMode;
  pointCount?: number;
...
export function DigitMorphField({
  ...
  const fluidModeRef = useRef<FluidVisualMode>(fluidMode);
  ...
  fluidModeRef.current = fluidMode;
  ...
  }, [debug, fluid, lifeState, onStats, reducedMotion, showPerimeter, theme]);
  ...
  return (
    <div
      ...
      data-show-perimeter={showPerimeter ? "true" : "false"}
      data-fluid={fluid ? "true" : "false"}
      data-fluid-mode={fluidMode}
      data-evidence-hash={evidenceHash ?? "waiting"}
      data-session-id={triplet?.session_id ?? "waiting"}
      data-window-id={triplet?.window_id ?? "waiting"}
...
function drawDigitField(
  ...
  const background = highContrast ? "#05070a" : fluid ? "#fcfbfd" : "#ffffff";
  ...
  if (fluid && !highContrast) {
    draws += drawFluidAtmosphere(
      context,
      width,
      height,
      particles,
  ...
  // When the optional presentation perimeter is hidden, the same centre body
  // gets a little more room without changing its signal mapping or geometry.
  ...
  const bodyScale =
    Math.min(frameHalfWidth, frameHalfHeight) *
    (showPerimeter ? 0.64 : fluid ? 0.92 : 0.78);
  const scaleX = Math.max(1, bodyScale * (fluid ? 1.38 : 1.18));
  const scaleY = Math.max(1, bodyScale * (fluid ? 1.08 : 1));
  const centerX = width * (fluid ? 0.59 : 0.5);
  const centerY = height * (fluid ? 0.52 : 0.5);
  ...
  const fontSize = fluid
    ? Math.max(5.2 * ratio, Math.min(10.5 * ratio, minSide / 54))
    : Math.max(8 * ratio, Math.min(17 * ratio, minSide / 35));
  context.font = `${fontSize}px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace`;
  context.textAlign = "center";
  context.textBaseline = "middle";
  const renderStride = 1;

  if (fluid && activity > 0.08) {
    draws += drawBodyHalo(
      context,
      centerX,
      centerY,
  ...
  const fluidSubset = fluid ? createFluidFieldPoints(current, lifeState, fluidMode) : null;
  for (let index = 0; index < current.length; index += 1) {
    if (index % renderStride !== 0) {
      continue;
    }
    const point = fluidSubset?.[index] ?? current[index];
    const alternatePoint = fluid ? point : alternate[index] ?? point;
    ...
    const candidateBlend = fluid
      ? 0
      : index % 2 === 0
        ? 0
        : params.disagreement_phase;
    ...
    let shapedX = mix(scatterX, candidateX + disagreementSplit, bodyFormation);
    let shapedY = mix(scatterY, candidateY, bodyFormation);
    const shapedZ = mix(scatterZ, candidateZ - disagreementSplit * 0.42, bodyFormation);
    if (fluid && params.active) {
      const flow = fluidDeformation(
        shapedX,
        shapedY,
        point.phase,
        index,
    ...
    context.globalAlpha = fluid ? Math.min(0.9, alpha * 1.48) : alpha;
    const rainbowIndex =
      (index +
    ...
    context.fillStyle =
      activity > 0.08
        ...
        : inactive;
    if (fluid && index % 5 === 0 && activity > 0.08) {
      const sparkle = context.createRadialGradient(x, y, 0, x, y, fontSize * 1.8);
      sparkle.addColorStop(0, "rgba(255,255,255,0.64)");
      sparkle.addColorStop(0.26, "rgba(248,207,255,0.22)");
      sparkle.addColorStop(1, "rgba(248,207,255,0)");
      context.save();
      context.globalAlpha = alpha * 0.38;
      context.fillStyle = sparkle;
      context.beginPath();
      context.arc(x, y, fontSize * 1.8, 0, Math.PI * 2);
      context.fill();
      context.restore();
      draws += 1;
    }
...
/**
 * The Home field deliberately abandons furniture-like outlines in favour of a
 * non-anthropomorphic, three-lobed fluid organism. The selected theme still
 * controls the normal field and state transitions; this is a presentation
 * projection for the fluid Home treatment only.
 */
function createFluidFieldPoints(
  points: readonly MutablePoint[],
  state: LifeStateId,
  mode: FluidVisualMode,
): MutablePoint[] {
  return points.map((point, index) => {
    const lobe = index % (mode === "pet_night" ? 2 : 3);
    const field = fluidModeField(mode, lobe);
    const angle = point.phase + (index * 0.61803398875 % 1) * Math.PI * 2;
    const radial = 0.15 + ((index * 0.38196601125) % 1) * 0.78;
    const breathing =
      (state === "doubt" ? 0.86 : state === "flow" ? 1.08 : 1) *
      (mode === "fall_drill" ? 0.8 : 1);
    return {
      ...point,
      x:
        field.centerX +
        Math.cos(angle) * field.radiusX * radial * breathing +
        Math.sin(angle * 3 + lobe) * 0.09,
      y:
        field.centerY +
        Math.sin(angle) * field.radiusY * radial * breathing +
        Math.cos(angle * 2 - lobe) * 0.07,
      z: point.z * 0.72 + Math.sin(angle * 2) * 0.14,
    };
  });
}

function fluidModeField(
  mode: FluidVisualMode,
  lobe: number,
  ...
  switch (mode) {
    case "routine":
      return {
        centerX: [-0.34, 0.16, 0.5][lobe] ?? 0,
        centerY: [0.02, -0.08, 0.09][lobe] ?? 0,
        radiusX: [0.68, 0.58, 0.48][lobe] ?? 0.5,
        radiusY: [0.31, 0.42, 0.3][lobe] ?? 0.34,
      };
    case "bathroom_timeout":
      return {
        centerX: 0.12 + (lobe - 1) * 0.04,
        centerY: (lobe - 1) * 0.03,
        radiusX: 0.36 + lobe * 0.035,
        radiusY: 0.36 + lobe * 0.035,
      };
    case "fall_drill":
      return {
        centerX: 0.1 + (lobe - 1) * 0.1,
        centerY: (lobe - 1) * 0.08,
        radiusX: 0.3 + lobe * 0.04,
        radiusY: 0.27 + lobe * 0.045,
      };
    case "pet_night":
      return {
        centerX: [-0.14, 0.45][lobe] ?? 0.18,
        centerY: [-0.03, 0.08][lobe] ?? 0,
        radiusX: [0.55, 0.42][lobe] ?? 0.45,
        radiusY: [0.22, 0.18][lobe] ?? 0.22,
      };
    case "ambient":
      return {
        centerX: [-0.42, 0.16, 0.46][lobe] ?? 0,
        centerY: [0.1, -0.18, 0.15][lobe] ?? 0,
        radiusX: [0.62, 0.54, 0.44][lobe] ?? 0.5,
        radiusY: [0.46, 0.54, 0.38][lobe] ?? 0.4,
      };
  }
}

function fluidDeformation(
  x: number,
  y: number,
  phase: number,
  index: number,
  ...
  const modeSpeed = mode === "fall_drill" ? 2.2 : mode === "bathroom_timeout" ? 0.62 : mode === "pet_night" ? 0.86 : 1;
  const drift = seconds * (0.22 + motion * 0.3) * modeSpeed + phase;
  const lobe = Math.sin(index * 0.077 + drift * 1.4);
  const wave = Math.sin(x * 7.5 + drift) * (0.038 + motion * 0.03) * (mode === "routine" ? 1.5 : 1);
  const ripple = Math.cos(y * 10 - drift * 1.15) * (0.026 + motion * 0.022) * (mode === "bathroom_timeout" ? 1.75 : 1);
  const statePulse =
    (lifeState === "doubt" ? 1.38 : lifeState === "flow" ? 1.18 : 0.92) *
    (mode === "fall_drill" ? 1.58 : mode === "pet_night" ? 0.78 : 1);
  return {
    x: x + wave * statePulse + lobe * 0.014,
    y:
      y +
      ripple * statePulse +
      Math.sin(x * 4 + drift * 0.55) * 0.018 +
      (mode === "fall_drill" ? Math.sin(index * 0.19 + drift * 2.4) * 0.02 : 0),
  };
}

/**
 * Home's richer material layer: it is derived only from the already-approved
 * visual parameters and seeded particles. It cannot add sensing claims or
 * alter the digit geometry below it.
 */
function drawFluidAtmosphere(
  context: CanvasRenderingContext2D,
  width: number,
  height: number,
  ...
  const motion = Math.min(1, Math.max(0, (params.particle_speed - 0.08) / 1.72));
  const activity = params.active ? 1 : 0.3;
  const isFocus = mode === "bathroom_timeout" || mode === "fall_drill";
  const centerY = height * (isFocus ? 0.5 : 0.49 + Math.sin(seconds * 0.09) * 0.025);
  const rightX = width * (isFocus ? 0.53 : 0.68 + Math.cos(seconds * 0.07) * 0.035);
  const fieldRadius = Math.max(width, height) * (0.48 + motion * 0.1);
  const wash = context.createRadialGradient(rightX, centerY, 0, rightX, centerY, fieldRadius);
  wash.addColorStop(0, `rgba(218, 132, 232, ${0.22 + motion * 0.1})`);
  wash.addColorStop(0.33, `rgba(255, 166, 188, ${0.18 + activity * 0.06})`);
  wash.addColorStop(0.67, "rgba(183, 191, 255, 0.14)");
  wash.addColorStop(1, "rgba(255,255,255,0)");
  context.save();
  context.fillStyle = wash;
  context.fillRect(0, 0, width, height);

  const waveColors = mode === "pet_night" ? [
    "rgba(135, 149, 246, 0.18)",
    "rgba(114, 211, 204, 0.17)",
    "rgba(201, 139, 244, 0.15)",
    "rgba(255, 189, 214, 0.12)",
  ] : [
    "rgba(255, 151, 180, 0.19)",
    "rgba(203, 131, 243, 0.20)",
    "rgba(119, 206, 207, 0.14)",
    "rgba(255, 202, 157, 0.15)",
  ];
  for (let layer = 0; layer < waveColors.length; layer += 1) {
    const baseY = isFocus
      ? height * (0.5 + (layer - 1.5) * 0.032)
      : height * (0.31 + layer * 0.13);
    const amplitude = height * (isFocus
      ? 0.022 + layer * 0.006 + motion * 0.012
      : 0.075 + layer * 0.009 + motion * 0.025);
    const phase = seconds * (mode === "fall_drill" ? 0.85 : mode === "pet_night" ? 0.22 : 0.16 + motion * 0.18) + layer * 1.8;
    const gradient = context.createLinearGradient(width * 0.2, baseY, width, baseY);
    gradient.addColorStop(0, "rgba(255,255,255,0)");
    gradient.addColorStop(0.38, waveColors[layer]);
    gradient.addColorStop(0.76, waveColors[(layer + 1) % waveColors.length]);
    gradient.addColorStop(1, "rgba(255,255,255,0)");
    context.strokeStyle = gradient;
    context.globalAlpha = 0.72 + activity * 0.2;
    context.lineWidth = Math.max(1, Math.min(width, height) * 0.003);
    context.beginPath();
    for (let step = 0; step <= 40; step += 1) {
      const progress = step / 40;
      const x = isFocus
        ? width * (0.08 + progress * 0.88)
        : width * (0.22 + progress * 0.83);
      const y =
        baseY +
        Math.sin(progress * Math.PI * 2.4 + phase) * amplitude +
        Math.sin(progress * Math.PI * 5.1 - phase * 0.7) * amplitude * 0.27;
  ...
  if (isFocus) {
    const pulseSpeed = mode === "fall_drill" ? 1.35 : 0.42;
    const pulse = (seconds * pulseSpeed) % 1;
    const rings = mode === "fall_drill" ? 7 : 5;
    context.save();
    for (let ring = 0; ring < rings; ring += 1) {
      const travel = (pulse + ring / rings) % 1;
      const radius = Math.min(width, height) * (0.08 + travel * (mode === "fall_drill" ? 0.44 : 0.36));
      context.globalAlpha = (1 - travel) * (mode === "fall_drill" ? 0.34 : 0.2);
      context.lineWidth = Math.max(1, Math.min(width, height) * (mode === "fall_drill" ? 0.0036 : 0.0026));
      context.strokeStyle = ring % 2 === 0 ? "rgba(246,111,176,0.72)" : "rgba(191,142,252,0.64)";
      context.beginPath();
      context.ellipse(rightX, centerY, radius * 1.14, radius * 0.72, seconds * 0.08 + ring * 0.15, 0, Math.PI * 2);
      context.stroke();
    }
    context.restore();
  }

  // A quiet dotted current makes the canvas feel denser without replacing the
  // signal-driven digits. It is intentionally background-only.
  for (let index = 0; index < Math.min(260, particles.length); index += 1) {
    const particle = particles[index];
    const lane = index % 7;
    const progress = (index * 0.61803398875) % 1;
    const x = width * (0.3 + progress * 0.72) + Math.sin(seconds * 0.24 + particle.phase) * width * 0.018;
    const y =
      height * (0.2 + lane * 0.095) +
      Math.sin(progress * Math.PI * 4 + seconds * 0.22 + particle.phase) * height * 0.025;
    const radius = (0.5 + particle.size * 0.14) * Math.min(2, window.devicePixelRatio || 1);
  ...
  // A subtle evidence bridge begins at the left field edge. It is visual
  // punctuation for the same-frame binding, not a new data channel.
  const bridgeY = height * (lifeState === "doubt" ? 0.54 : isFocus ? 0.5 : 0.5);
  const bridge = context.createLinearGradient(0, bridgeY, width * 0.34, bridgeY);
  bridge.addColorStop(0, "rgba(255,255,255,0)");
  bridge.addColorStop(0.48, "rgba(255,255,255,0.86)");
  bridge.addColorStop(1, "rgba(224,182,255,0)");
  context.globalAlpha = 0.72;
  context.strokeStyle = bridge;
  context.lineWidth = Math.max(1.5, Math.min(width, height) * 0.0032);
  context.beginPath();
  context.moveTo(0, bridgeY);
  context.bezierCurveTo(
    width * 0.1,
    bridgeY - height * 0.018,
    width * 0.19,
    bridgeY + height * 0.022,
    width * 0.34,
    bridgeY + Math.sin(seconds * 0.45) * height * 0.012,
  );
  context.stroke();
  context.restore();
  return 1 + waveColors.length + Math.min(260, particles.length) + 1 + (isFocus ? (mode === "fall_drill" ? 7 : 5) : 0);
}

function drawBodyHalo(
  context: CanvasRenderingContext2D,
  centerX: number,
  centerY: number,
  size: number,
  ...
  const motion = Math.min(1, Math.max(0, (params.particle_speed - 0.08) / 1.72));
  const pulse = 1 + Math.sin(seconds * (0.5 + motion * 0.35)) * 0.07;
  const radius = size * (mode === "fall_drill" ? 0.52 : mode === "bathroom_timeout" ? 0.56 : 0.76 + motion * 0.12) * pulse;
  const halo = context.createRadialGradient(centerX, centerY, 0, centerX, centerY, radius);
  halo.addColorStop(0, "rgba(255,255,255,0.12)");
  halo.addColorStop(0.33, "rgba(246,168,220,0.075)");
  halo.addColorStop(0.68, `${hexToRgba(accent, 0.055)}`);
  halo.addColorStop(1, "rgba(255,255,255,0)");
  context.save();
  context.fillStyle = halo;
  context.beginPath();
  context.ellipse(centerX, centerY, radius * 1.18, radius * 0.84, 0, 0, Math.PI * 2);
  context.fill();
  context.save();
  context.globalAlpha = 0.28 + motion * 0.12;
  context.lineWidth = Math.max(1, size * 0.006);
  const ringCount = mode === "fall_drill" ? 5 : mode === "bathroom_timeout" ? 4 : 3;
  for (let ring = 0; ring < ringCount; ring += 1) {
    const ringRadius = radius * (0.47 + ring * 0.16);
    const contour = context.createLinearGradient(
      centerX - ringRadius,
      centerY,
      centerX + ringRadius,
      centerY,
    ...
    contour.addColorStop(0.28, "rgba(255,177,208,0.46)");
    contour.addColorStop(0.58, hexToRgba(accent, 0.54));
    contour.addColorStop(1, "rgba(120,208,215,0)");
    context.strokeStyle = contour;
    context.beginPath();
    context.ellipse(
      centerX + Math.sin(seconds * 0.18 + ring) * size * 0.035,
      centerY + Math.cos(seconds * 0.22 + ring) * size * 0.024,
      ringRadius * (mode === "routine" ? 1.65 : 1.3),
      ringRadius * (mode === "routine" ? 0.44 : 0.62),
      Math.sin(seconds * 0.12 + ring) * 0.22,
      0,
      Math.PI * 2,
    );
    context.stroke();
...
function hexToRgba(hex: string, alpha: number): string {
  ...
  if (normalized.length !== 6) return `rgba(193,128,225,${alpha})`;
