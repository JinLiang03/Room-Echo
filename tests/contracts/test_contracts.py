...
MODEL_BY_FILE: dict[str, type[BaseModel]] = {
    ...
    "agent_action_decisions.json": AgentActionDecision,
    "simulated_care_scenarios.json": SimulatedCareScenario,
...
def test_role_projection_contracts_are_exported_and_populated() -> None:
    ...
    assert "AgentActionDecision" in result_schema["$defs"]
    ...
    assert all(result.action_decision is not None for result in results)
