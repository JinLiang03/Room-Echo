...
test("home consumes one accelerated care frame as a bound digital life", async ({
  page,
}) => {
  await page.goto("/#/home");
  await expect(page.locator(".digit-field-canvas")).toBeVisible();
  await expect(page.getByText("SIM · CARE")).toBeVisible({ timeout: 30_000 });
  ...
  const publicAgent = page.locator('[data-public-agent="room-echo"]');
  await expect(publicAgent).toHaveCount(1);
  await expect(publicAgent).not.toHaveAttribute("data-cycle-id", "waiting", {
    timeout: 60_000,
  });
  await expect(publicAgent).not.toHaveAttribute("data-evidence-hash", "waiting");
  await expect(publicAgent).not.toHaveAttribute("data-window-id", "waiting");
  ...
  await expect(page.locator(".action-suggestion").first()).not.toHaveAttribute(
    "data-action-status",
    "pending",
    { timeout: 60_000 },
  );
...
test("happy replay: signals flow and council debate reaches the UI", async ({
  ...
  await page.goto("/#/council?audit=1");
